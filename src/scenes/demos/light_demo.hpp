...
inline Scene light_demo() {
    ...
    s.objects.add(std::make_shared<Sphere>(point(1,0,-1000,0), 1000, std::make_shared<Lambertian>(pertext)));
    s.objects.add(std::make_shared<Sphere>(point(1,0,2,0), 2, std::make_shared<Lambertian>(pertext)));

    auto difflight = std::make_shared<DiffuseLight>(color(4,4,4));
    s.objects.add(std::make_shared<Sphere>(point(1,0,7,0), 2, difflight));
    s.objects.add(std::make_shared<Quad>(point(1,3,1,-2), point(1,2,0,0), point(1,0,2,0), difflight));
