...
inline Scene triangle_demo() {
    ...
    s.controls.set_camera_pos(point(1,10,-50, 10), point(1,0,0,0), point(1,0,0,1));
    ...
    s.objects.add(std::make_shared<Sphere>(point(1,0,0,3), 7, difflight));
    ...
    for (int i = 0; i < 10; i++) {
        s.objects.add(std::make_shared<Triangle>(
            point(1, (i-5)*5, (i-5)*5, 0),
            point(1, ((i-5)*5)+2, ((i-5)*5)+2, 0),
            point(1, ((i-5)*5)+1, ((i-5)*5)+1, 5),
            l));
    }
