...
Scene triangle_demo() {
    ...
    s.objects.add(std::make_shared<Sphere>(point(0,0,20), 2, difflight));

    auto ground_material = std::make_shared<Lambertian>(color(0.5, 0.5, 0.5));
    s.objects.add(std::make_shared<Sphere>(point(0,0,-1000), 1000, ground_material));
    auto albedo = color(255.0/256, 0.0, 0.0);
    auto mat = std::make_shared<Metal>(albedo, 0.5);

    for (int i = 0; i < 10; i++) {
        s.objects.add(std::make_shared<Triangle>(point((i-5)*5, (i-5)*5, 0), point((i-5)*5+2, (i-5)*5+2, 0), point((i-5)*5+1, (i-5)*5+1, 5), mat));
    }
